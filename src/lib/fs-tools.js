...
const full_mediaJsonArray_path = join(dataFolderPath, "media.json") // =====>this is the shortest way getting dynamic path of the "blogpost.json" path in difrent opreating system
// const full_Authors_JSONpath = join(dataFolderPath, "media.json")
...
// const full_mediaJsonArray_path = join(parentdirrctory, "book.json")

//=========FUNCTION TO READ AND WRITE THE JSON FILE===========
export const readMediaJson = () => readJSON(full_mediaJsonArray_path) // READING the json data and JSON.parse convert buffer to readable stream
export const writeMediatJson = (content) => writeJSON(full_mediaJsonArray_path, content) //writting on blogpost.json and stringfy as json mode
...
export const getBooksReadableStream = () => createReadStream(full_mediaJsonArray_path)
